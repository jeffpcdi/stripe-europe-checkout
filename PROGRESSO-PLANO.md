...
- **Sessão 14 (atual):** A nova aba Conversões revelou o motivo real do flood: **`Cannot convert argument to a ByteString (char 0 = 65533)`** em TODO disparo. Causa: um Access-Token com o caractere `0xFFFD` (o "�" de um cadastro com encoding errado) — o `fetch` do Node exige header Latin-1, então crashava, caía no catch e **re-enfileirava para sempre** (fila 187, 15% ok, EMQ no chão). **Fix em 3 camadas:** (1) `sendToPixel` (tiktok-events) saneia o token para ASCII imprimível (`headerSafeToken`) antes do header — recupera token com caractere solto; se sobrar vazio, retorna `BAD_TOKEN` claro **sem re-enfileirar**; (2) guard extra: `TypeError` (erro determinístico de header) não re-enfileira; (3) `pixel-store` saneia `pixelCode`/`accessToken`/`testEventCode` no cadastro (`cleanAscii`, era só `trim`). Teste `test/capi-token-sanitize.test.js` (6 asserts: token sujo → saneado e disparado; token lixo → BAD_TOKEN sem crash/requeue). **Também corrigido o bug de DUPLICAÇÃO (40002 "budget_mode is required"):** `buildAdGroupCopyArgs` (ads-provider) não setava `budget_mode` quando a origem era **CBO** (grupo INFINITE) sem override → o `create_tiktok_adgroup` exige o campo sempre. Agora cai em `BUDGET_MODE_INFINITE` (grupo herda o orçamento da campanha). Regressão em `test/ads-duplicate.test.js` (caso 7 CBO). Validação: `npm test` verde, syntax OK. **Duplo-disparo de Purchase (3 reais → 6 no TikTok):** diagnosticado como pixel nativo do checkout disparando CompletePayment no navegador junto da nossa CAPI (event_ids diferentes → sem dedup) — não é bug do nosso código (nosso px.js só dispara ViewContent client-side; o event_id da CAPI é determinístico por pedido); orientação dada ao usuário.
- **Sessão 13:** Feed de Gateways mostrava **"generic erro em 1/2"** repetido + badge **"Fila acumulada (22)"**. Diagnóstico: cada venda dispara nos 2 pixels e 1 falha na CAPI (aposta: token inválido), mas o recibo guardava só o agregado — o motivo por pixel vivia no log da aba Pixels. **Fix (backend aditivo):** `sendToPixel` (tiktok-events) devolve `pixel`/`pixelName` em TODO retorno (sucesso, skip, falha de rede) + o ramo de rejeição do `allSettled` no `dispatchToAll`; `processConversion` (server.js) grava `receipt.capi = [{pixel,ok,code,message}]`. **UI:** o feed de Gateways expande com ✓/✗ por pixel + motivo do TikTok, e CTA "Corrigir token do pixel" quando o motivo casa `/access.?token|40105|unauthor/`; badge "Fila acumulada" vira Link para `/conversions?tab=gateways`. **Fusão Gateways+Pixels numa aba "Conversões"** (a pedido — trabalham juntos): nova `ConversionsView` (`components/conversions/`) com 2 segmentos `?tab=gateways|pixels` (reusa as views existentes) + strip "Fluxo da conversão" (Venda→Fila→CAPI, 3 métricas ao vivo de `useGateways`/`useOps`/`usePixelHealth`); card de gateway ganhou linha "Vendas disparam em: X, Y" (espelha a regra do dispatch) + aviso âmbar quando ZERO pixels recebem; `/gateways` e `/pixels` viram redirects (padrão do catalog); navegação funde os 2 itens em "Conversões"; `tourForPath` segue o segmento. Nova suíte `test/capi-receipt.test.js` (6 asserts, stub pixel-store+redis+fetch: 2 pixels, um code 0 e um 40105). Validação: `npm test` verde, `tsc` + `next build` limpos (rota `/conversions` + redirects), Express `/healthz` 200. **Nota honesta:** o motivo exato do erro em produção só aparece nos logs de lá — agora aparece direto no feed. Docs: `CLAUDE.md` §19.3.
- **Sessão 12:** Publicar catálogo passou a dar **502 "Falha na API"** (o Vercel saiu; agora quem recusa é a chamada real ao TikTok/Pipeboard). Diagnóstico: o `parseApiError` só mostra "Falha na API (502)" cru quando o corpo do 502 **não é o JSON do app** (timeout/erro de borda do Railway na cadeia Pipeboard) — não reproduzível no sandbox. Como o sucesso do publish direto depende de BC/permissão/timeout, fiz as duas frentes pedidas: **(1) publish honesto e não-bloqueante** — `parseApiError` (`lib/api.ts`) passou a dar mensagem+hint acionáveis para 502/503/504/5xx e corpo não-JSON; `handleSyncTiktok` usa `ApiError.display`, seta `publishFailed` e o toast vira "use o CSV"; o catch do `/sync-tiktok` (ads-routes) loga a razão real (`[catálogo] Falha ao publicar…`) no feed de Operações. **(2) CSV pronto para conversão como caminho principal** — `GET /export.csv` passou a servir **só produtos válidos** (import-ready; `?all=1` para debug); novo componente `ManualUploadGuide` (passo a passo: baixar → Catalog Manager → importar → campanha Product Sales/conversão) sempre visível quando há produtos válidos, + bloco de fallback em destaque quando o publish automático falha. Testes: +3 asserts em `test/ads-catalog-campaign.test.js` (export.csv filtra válidos, `?all=1`, log no catch). Validação: `npm test` verde, `tsc` + `next build` limpos, boot OK (`export.csv`/`sync-tiktok` 401 sem sessão, `/healthz` 200). **Limite honesto:** o publish direto pode dar 502 por BC/permissão/timeout — o CSV é o caminho garantido. Docs: `PLANO-VERIFICACAO-ADS.md` (passo 9d), `CLAUDE.md` §19.3.
- **Sessão 11:** Remoção do **Vercel Blob** (o app roda no Railway; publicar catálogo caía em 503 "Armazenamento não configurado"). **Feed do catálogo** passou a ser servido pelo próprio app: nova coluna `feed_token` + `ensureFeedToken`/`getCatalogByFeedToken` (`ads-catalog-store.js`), rota pública `GET /feed/:token.csv` (`server.js`) que regenera o CSV do Neon a cada fetch (o TikTok re-puxa sozinho), e `publishCatalogFeed` agora monta `feedUrl = publicOrigin + '/feed/<token>.csv'` (sem Blob). **Uploads de criativo** (vídeo/imagem) e a **biblioteca** foram para disco: novo módulo `ads-storage.js` (`UPLOAD_DIR` = Volume do Railway via `RAILWAY_VOLUME_MOUNT_PATH`, senão `data/uploads`; `publicOrigin` de `PRIMARY_HOST`/`RAILWAY_PUBLIC_DOMAIN`/host, sem porta; `safeSegment`/`safeName` anti-traversal), servidos por `express.static('/uploads')`; `POST /api/ads/upload` grava em disco e `GET/DELETE /api/ads/library` leem/apagam do dir da conta. `@vercel/blob` **removido do `package.json`** e de todo `require`. Guard de domínio: `/feed/` e `/uploads/` no allowlist. Nova suíte `test/ads-storage.test.js` (22 asserts: publicOrigin env×header sem porta, UPLOAD_DIR volume×fallback, sanitização, feed_token exportado, ausência de Blob, rotas públicas). Validação: `npm test` verde; boot sem `BLOB_READ_WRITE_TOKEN` → `/healthz` 200, `/feed/<token>.csv` 404 gracioso sem Neon, `/uploads/<acc>/<arquivo>` serve o binário (content-type `video/mp4`), `POST /api/ads/upload` 401 sem sessão, `grep @vercel/blob` = 0 funcional. **Ops do usuário:** setar `PRIMARY_HOST` (feed/upload precisam de URL pública absoluta) e anexar um Volume no Railway para uploads duráveis. Docs: `CLAUDE.md` (§12 envs + §15 estrutura + §19.3), `PLANO-VERIFICACAO-ADS.md` (passo 9).
- **Sessão 10:** Menos dependência do Ads Manager, foco no catálogo. Apuração: "upload de vídeo novo" JÁ existia (passo 4 do criador: URL/arquivo até 500 MB/biblioteca) — não refeito. **Parte A — direcionamento na criação:** `buildCreatePayload` (ads-routes) + `createFullAd` (ads-provider) ganharam gênero (`GENDER_MALE/FEMALE`), interesses (`interest_category_ids`, cap 20, IDs numéricos) e posicionamento (`PLACEMENT_TYPE_AUTOMATIC`×`NORMAL`+placements); nova rota leitura `GET /api/ads/targeting/interests` → `provider.listInterestCategories` (cache 24h); UI no passo Público do `create-ad-panel.tsx` (gênero, posicionamento, busca de interesses com chips) + revisão + templates. **Parte B — lançar campanha de catálogo (DPA):** `provider.createCatalogCampaign` (composto à prova de órfãos, PRODUCT_SALES + `product_source:CATALOG` + todos os produtos, nasce PAUSED, reusa `resolveBudgetPlan`/`resolveLocationIds`/`pickAdIdentity`) + rota `POST /api/ads/catalogs/:catalogId/campaign` (killSwitch/dry-run/NOT_SYNCED, auditoria) + botão "Criar campanha deste catálogo" no `TiktokStatusPanel` (mini-form nome/orçamento/país/ABO-CBO). Tipos: targeting no `AdsTemplatePayload`, `AdsCatalogCampaignResponse`; hooks `useAdsInterests`/`adsCreateCatalogCampaign`. Novas suítes `test/ads-targeting.test.js` (11) e `test/ads-catalog-campaign.test.js` (21). Validação: `npm test` completo verde, `tsc --noEmit` + `next build` limpos, boot Express (`/healthz` 200, `POST /catalogs/:id/campaign` e `GET /targeting/interests` 401 sem sessão). **Ressalva honesta:** o Pipeboard não tem tool dedicada de campanha de catálogo — uso as genéricas com os campos de catálogo; se a API recusar algum, a resposta traz `step`/motivo e o parcial fica pausado. Prova real (parte B) fica no runbook de produção. Docs: `PLANO-VERIFICACAO-ADS.md` (passos 9b/9c), `CLAUDE.md` §19.3.
- **Sessão 9:** Roadmap C do TikTok Ads (C1 + C3, a pedido — "tudo menos o resumo semanal/C2"). **C1 — automação sobre Smart+:** `ads-sync.syncAdvertiser` passou a mesclar as campanhas Smart+ no MESMO espelho como nós `campaignKind:'smart_plus'` (best-effort; métricas vêm do `collectDaily` de `AUCTION_CAMPAIGN` existente, sem chamada extra); `ads-provider.getDashboardTree` marca os nós de leilão com `campaignKind:'auction'`; o motor (`executeRuleAction` + novo `setCampaignStatusByKind` + `runScheduleSweep`) roteia PAUSA/ativação de Smart+ para `provider.setSmartPlusCampaignStatus`, e `runRulesSweep` PULA regras de orçamento/escala em Smart+ com motivo (o Pipeboard não expõe tool de orçamento de Smart+). **C3 — auto-recurso de Smart+ reprovado:** alerta opt-in `autoAppealSmartPlus` (`ALERT_DEFAULTS` + `PUT /api/ads/alerts`) → `runAlertSweep` chama `provider.appealSmartPlusAd` 1×/anúncio com cooldown de 7d (`APPEAL_COOLDOWN_MS`), respeitando kill switch e Modo teste (dry-run só audita). UI: checkbox aninhado no painel de alertas + microcopy do Piloto Protetor ("cobre Smart+"). Tipos: `campaignKind` em `AdsTreeCampaign`, `autoAppealSmartPlus` em `AdsAlertsConfig`. Nova suíte `test/ads-smart-plus-automation.test.js` (28 asserts: roteamento por tipo com provider stubado + cooldown/guardrails do auto-appeal + regex de sync/skip/rota); janela do assert Smart+ em `ads-automation-cpc.test.js` alargada (400→900). Validação: `npm test` completo verde, `tsc --noEmit` + `next build` limpos, boot do Express (`/healthz` 200, `PUT /api/ads/alerts` 401 sem sessão). **Limites documentados:** Smart+ só pausa (sem escala via API); pausa por métrica depende de o TikTok reportar Smart+ no nível de campanha; nada exercitado ao vivo (sandbox sem `PIPEBOARD_API_KEY`) — prova fica no runbook de produção. Docs: `PLANO-VERIFICACAO-ADS.md` (C1/C3 marcados ENTREGUE + runbook passos 7/8) e `CLAUDE.md` §19.3 atualizados.


### Revisão de interface — 2026-09-07
- Navegação, indicadores, globo, automações, modais e textos simplificados; contratos e evidências em
