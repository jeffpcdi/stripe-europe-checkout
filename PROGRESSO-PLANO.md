...
Pendente do lote (UI ampla, próxima fatia): 175, 182–190 (formato de erro na UI de cada aba, toaster global, timeouts de UX, a11y de modais/foco).

Evidência: `node --check` limpo nos 3 módulos, `next build` limpo (type-check incluído, todas as rotas prerenderizadas), 5/5 suítes de teste passando, `getJudgeLatency()` conferido em runtime.

## Fila de execução (próximos)

Ordem recomendada pelo plano (bugs → durabilidade → segurança → valor → refino → DX):

1. Leva 4 (141–200) — 161–168/204–206/210 (transparência do cloak) + 176–181 (backend/API: cache negativo ASN, latência do cloaker, rate-limit dos testes, verify-url endurecido, sanitização, contrato de erro) concluídos; faltam 141–160, 169–175, 182–203, 207–209, 211–240
