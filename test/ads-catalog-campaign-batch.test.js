...
console.log('Rota — guardrails do campaign-batch');
{
  ...
  ok(/hasExplicitCount \? Number\(body\.count\) : videoUrls\.length/.test(body), 'quantidade explícita não é substituída pela quantidade de vídeos');
  ok(/videoUrls\[index % videoUrls\.length\]/.test(body), 'um vídeo pode ser reutilizado e vários são distribuídos pelo lote');
...
console.log('Dialog — lote rápido sem CSV nem configuração repetida');
{
  ...
  ok(/adsUpload\(file, 'video'\)/.test(dialog) && /videoUrls: videos\.map/.test(dialog), 'lote recebe um ou vários vídeos sem etapa manual no Ads Manager');
  ...
  ok(/Perfil de Investimento/.test(dialog) && /Máxima entrega/.test(dialog) && /cost_cap/.test(dialog), 'opções avançadas expõem estratégia de investimento sem poluir o fluxo principal');
  ok(/deliveryMode: 'standard'/.test(dialog), 'lote usa entrega padrão segura');
  ...
  ok(/catalogCostCap/.test(dialog), 'interface só mostra Cost Cap quando confirmado pelo schema vivo');
  ...
  ok(/Criar campanhas em massa/.test(wizard) && /disabled=\{!connectorReady \|\| !ready \|\| activeRuns\.length > 0\}/.test(wizard) && /open=\{dialogOpen\}/.test(wizard), 'botão principal de lote só abre quando Product Link e catálogo estão prontos');
  const detail = fs.readFileSync(path.join(__dirname, '..', 'dashboard', 'components', 'ads', 'catalog-detail.tsx'), 'utf8');
  ok(detail.indexOf('<CatalogCampaignWizard') < detail.indexOf('Produtos e sincronização'), 'ação de campanhas aparece antes dos controles secundários de produtos');
  ok(/<details[\s\S]*Produtos e sincronização/.test(detail), 'produtos e sincronização ficam recolhidos em um painel secundário');
  const routes = fs.readFileSync(path.join(__dirname, '..', 'ads-routes.js'), 'utf8');
