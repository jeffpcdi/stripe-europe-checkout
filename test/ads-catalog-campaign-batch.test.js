...
console.log('Rota — guardrails do campaign-batch');
{
  ...
  ok(/prepared\.spec\.pixelId,[\s\S]*prepared\.spec\.bidStrategy[\s\S]*prepared\.spec\.deliveryMode[\s\S]*prepared\.spec\.identityId[\s\S]*(prepared\.spec\.videoUrl|videoUrls)[\s\S]*namePrefix/.test(body), 'fallback idempotente muda com vídeo, lance, entrega, perfil ou nome');
...
console.log('Dialog — lote rápido sem CSV nem configuração repetida');
{
  ...
  ok(/max=\{MAX_COUNT\}/.test(dialog) && /Quantidade de campanhas/.test(dialog), 'quantidade personalizável mantém limite de 50');
  ok(/campanhas serão conferidas e ativadas automaticamente/.test(dialog), 'aceite distingue fila de ativação concluída');
  ...
  ok(/productScope: 'all'/.test(dialog) && /autoActivate: true/.test(dialog), 'configuração automática continua no contrato de criação');
  ...
  ok(/Mais configurações/.test(dialog) && /advancedSummary/.test(dialog), 'resumo fechado mostra opções alteradas');
  ...
  const modal = fs.readFileSync(path.join(__dirname, '../dashboard/components/ui/modal.tsx'), 'utf8');
  ok(/overflow-y-auto/.test(modal) && /footer && <footer/.test(modal), 'modal compartilhado mantém corpo rolável e rodapé fixo');
  ok(/aria-label="Fechar"/.test(modal), 'modal compartilhado mantém saída acessível');
  ok(/submitHint \|\| submissionError/.test(dialog), 'ação bloqueada explica o próximo passo');
