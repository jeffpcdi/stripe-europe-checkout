...
assert(server.includes("const tokenAcc = companionApiAccount(req);"), 'widget deve aceitar somente o token dedicado do Companion')
...
assert(server.includes("app.delete('/api/companion/device/:id', dashboardAuth"), 'dashboard deve permitir remover iPhone pareado sem expor o token do Companion')
assert(server.includes("app.get('/api/reports/daily/preview', dashboardAuth"), 'brief diário deve oferecer prévia autenticada pela mesma fonte de verdade')
assert(server.includes('buildDailyExecutiveBrief(accId)') && server.includes('buildDailyExecutiveBrief(req.account.id)'), 'scheduler e prévia devem compartilhar a mesma composição de briefing')
...
assert(!widgetBlock.includes('publicApiAccount(req)'), 'token público de BI não deve ganhar acesso a lucro/gasto do widget')
...
assert(companionCard.includes('/api/companion/device/') && companionCard.includes('Remover aparelho'), 'iPhones pareados devem ser removíveis individualmente pela dashboard')
...
assert(widgetSwift.includes('snapshot.generatedAt') && widgetSwift.includes('Atualizado há'), 'widget deve mostrar a idade real do snapshot em vez do horário de renderização')
...
assert(widgetSwift.includes('.widgetURL(primaryURL(entry.snapshot))'), 'widget executivo deve usar destino contextual')
assert(widgetSwift.includes('Label("Vendas"') && widgetSwift.includes('Label("TikTok Ads"'), 'widget grande deve oferecer atalhos diretos para superfícies operacionais')
assert(widgetSwift.includes('snapshot.attention.contains("spend_without_sales")'), 'widget com gasto sem venda deve abrir o TikTok Ads para resolução')
