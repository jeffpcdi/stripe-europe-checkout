...
const overview = fs.readFileSync(path.join(root, 'dashboard/components/overview/overview-view.tsx'), 'utf8')
...
assert(overview.includes('overviewCampaignStatus') && overview.includes("label: 'Rejeitada'") && overview.includes("label: 'Em análise'"), 'Visão Geral não deve colapsar erro/revisão de campanha em pausada')
assert(domains.includes("{ label: 'Pronto', done: ready }") && !domains.includes("{ label: 'HTTPS', done: ready }"), 'progresso de domínio não deve repetir dois passos com a mesma condição')
