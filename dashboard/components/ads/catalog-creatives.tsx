...
import { Film, Loader2, Plus, RotateCcw, X } from 'lucide-react'
  ...
  return (
    <section className="catalog-creatives-compact" aria-label="Criativos do catálogo" aria-busy={busy}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <h3 className="flex items-center gap-1.5 text-xs font-semibold text-foreground">
            <Film className="size-3.5 text-primary" aria-hidden="true" /> Criativos
          </h3>
          <span className="text-[10px] tabular-nums text-muted-foreground">{value.length + pending.length}/50</span>
        </div>
        <button
          type="button"
          className="btn-ghost !min-h-0 h-7 text-[11px] px-2.5 gap-1 text-primary"
      ...
      {!value.length && !pending.length ? (
        <p className="mt-2 text-[11px] text-muted-foreground">Nenhum vídeo vinculado. Adicione MP4 ou MOV.</p>
      ) : (
        <div className="catalog-creatives-list mt-2 max-h-52 overflow-y-auto">
          {value.map(creative => (
            <div key={creative.id} className="catalog-creatives-row">
              ...
              <button type="button" className="btn-ghost !min-h-0 !size-6 p-0 text-muted-foreground hover:text-error" disabled={disabled || busy || !!removing} onClick={() => void remove(creative)} aria-label={`Remover ${creative.name}`}>
          ...
          {pending.map(item => (
            <div key={item.id} className={`catalog-creatives-row ${item.status === 'error' ? 'catalog-creatives-row--error' : ''}`}>
              ...
              <span className="min-w-0 flex-1 truncate text-[11px] text-foreground" title={item.file.name}>{item.file.name}</span>
              <span className={`text-[10px] ${item.status === 'error' ? 'text-error' : 'text-muted-foreground'}`}>{item.error || (item.status === 'saving' ? 'Salvando…' : item.status === 'queued' ? 'Na fila' : 'Enviando…')}</span>
              {item.status === 'error' && <button type="button" className="btn-ghost !min-h-0 !size-6 p-0" disabled={busy || disabled} onClick={() => void upload([item])} aria-label={`Reenviar ${item.file.name}`}><RotateCcw className="size-3" /></button>}
              {item.status === 'error' && <button type="button" className="btn-ghost !min-h-0 !size-6 p-0" disabled={busy || disabled} onClick={() => setPending(rows => rows.filter(row => row.id !== item.id))} aria-label={`Remover ${item.file.name}`}><X className="size-3" /></button>}
            </div>
          ))}
        </div>
      )}
