...
export function CatalogCampaignWizard({
  ...
  const activeRuns = useMemo(() => runs.filter((run) => ACTIVE_STATUSES.includes(run.status)), [runs])
  const activeRunIds = useMemo(() => new Set(activeRuns.map((run) => run.id)), [activeRuns])
  const currentRuns = activeRuns.length > 0 ? activeRuns.slice(0, 3) : runs.slice(0, 1)
  const previousRuns = runs.filter((run) => !activeRunIds.has(run.id) && !currentRuns.some((current) => current.id === run.id)).slice(0, 9)
  const blockers = capabilities?.blockers ?? []

  return (
    <section id="catalog-campaign-wizard" className="scroll-mt-4 rounded-xl border border-primary/45 bg-primary/[0.035] p-4 shadow-[0_0_32px_rgba(37,244,238,0.08)]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-1">
          <h3 className="flex items-center gap-2 text-sm font-bold text-foreground"><Rocket className="size-4 text-primary" /> Campanhas de catálogo</h3>
          <p className="text-[11px] text-muted-foreground">Crie de 1 a 50 campanhas Product Link em uma única operação.</p>
        </div>
        {capabilities === null
          ...
          : (
            <button
              type="button"
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-extrabold text-black shadow-[var(--glow-cyan)] transition-all hover:-translate-y-px hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
              onClick={() => setDialogOpen(true)}
              disabled={!connectorReady || !ready || activeRuns.length > 0}
            >
              {activeRuns.length > 0 ? <Loader2 className="size-4 animate-spin" /> : <Rocket className="size-4" />}
              {activeRuns.length > 0 ? `${activeRuns.length} em andamento` : 'Criar campanhas em massa'}
            </button>
          )}
      </div>
      ...
      {currentRuns.length > 0 && (
        <div className="mt-4 space-y-2 border-t border-border pt-4">
          {activeRuns.length > 3 && <p className="text-[10px] text-primary">Mostrando 3 de {activeRuns.length} campanhas em andamento.</p>}
          {currentRuns.map((run) => (
