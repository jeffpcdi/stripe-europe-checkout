...
export function KpiRow({ currency, active, adAccountId, fromDate, toDate, timeZone }: {
  ...
  const roasBadge = (() => {
    ...
    if (roas >= 2.0) return { label: 'Excelente (2×+)', cls: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/25' }
    if (roas >= 1.0) return { label: 'Lucrativo', cls: 'text-cyan-400 bg-cyan-500/10 border-cyan-500/25' }
    return { label: 'Abaixo da meta', cls: 'text-amber-400 bg-amber-500/10 border-amber-500/25' }
  })()

  const salesBadge = salesData && salesData.sales > 0 ? {
    label: `${salesData.sales} ${salesData.sales === 1 ? 'venda confirmada' : 'vendas confirmadas'}`,
    cls: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/25',
  } : null

  const cards = [
    { icon: Wallet, label: 'Gasto em ADS', value: cur?.spend ?? null, money, detail: 'Todas as campanhas da conta', badge: null, theme: 'pink' },
    { icon: ReceiptText, label: 'Receita', value: revenue, money: revenueMoney, detail: salesData ? `${salesData.sales} ${salesData.sales === 1 ? 'venda atribuída' : 'vendas atribuídas'}` : 'Vendas atribuídas aos anúncios', badge: salesBadge, theme: 'green' },
    { icon: ChartNoAxesCombined, label: 'Retorno', value: roas, money: '', detail: 'Receita ÷ investimento (ROAS)', badge: roasBadge, theme: 'cyan' },
    { icon: Target, label: 'Custo por venda', value: salesData?.currencyMismatch ? null : salesData?.cpa ?? null, money, detail: 'Investimento ÷ vendas', badge: null, theme: 'amber' },
  ]

  // Métricas derivadas
  const avgCpc = cur?.clicks && cur?.spend && cur.clicks > 0 ? cur.spend / cur.clicks : null
  const avgTicket = revenue && salesData?.sales && salesData.sales > 0 ? revenue / salesData.sales : null
        ...
        <div className="overview-metrics stagger-fade">
          {cards.map((card, index) => (
            <article
              ...
              className="overview-metric stagger-fade"
            ...
            >
              ...
              <div className="overview-metric-main">
                ...
                {card.badge && (
                  <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${card.badge.cls}`}>
                    {card.badge.label}
                  </span>
                )}
              </div>
        ...
        <details className="group text-xs text-muted-foreground">
          <summary className="w-fit cursor-pointer rounded-md py-1 hover:text-foreground select-none transition-colors font-medium">
            <span className="inline-flex items-center gap-1.5">
              ...
              Métricas detalhadas de entrega e tráfego
            </span>
          </summary>
